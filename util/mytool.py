...
def select_3_confident_region(out1,out2,out3,noise_label,thed=0.5):
    
    out1_soft = torch.softmax(out1,1)
    out2_soft = torch.softmax(out2,1)
    out3_soft = torch.softmax(out3,1)

    label1 = torch.argmax(out1_soft,1)
    label2 = torch.argmax(out2_soft,1)
    label3 = torch.argmax(out3_soft,1)
    ...
    # assessment uncertainty for GMM
    logit3, _ = torch.topk(out3_soft, k=2, dim=1)
    logit_max3 = logit3[:, 0]
    logit_sec3 = logit3[:, 1]
    un3 = -(logit_max3 * torch.log(logit_max3) + logit_sec3 * torch.log(logit_sec3))

    cross_mask1 = (un1 < un2) & (un2 < un3) | (un1 < un3) & (un3 < un2)#&(logit_max1>0.90)
    cross_mask2 = (un2 < un1) & (un1 < un3) | (un2 < un3) & (un3 < un1)#&(logit_max2>0.90)
    cross_mask3 = (un3 < un2) & (un2 < un1) | (un3 < un2) & (un2 < un1)

    mix_label = torch.zeros_like(noise_label).long().cuda()
    
    mix_label[cross_mask1==True] = label1[cross_mask1==True]
    mix_label[cross_mask2==True] = label2[cross_mask2==True]    
    mix_label[cross_mask3==True] = label3[cross_mask3==True]

    low_region = (logit_max1<thed) & (logit_max2<thed) & (logit_max3<thed)
