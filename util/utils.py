...
def color_map(dataset='pascal'):
    ...
    elif dataset == 'treecanopy':
        # UTC
        cmap[0] = np.array([0, 0, 0])  # Background
        cmap[1] = np.array([255, 255, 255])      # Tree

    elif dataset == 'vaihingen':
        # Vaihingen
        cmap[0] = np.array([255, 255, 255])  # Impervious surfaces
        cmap[1] = np.array([0, 0, 255])      # Building
        cmap[2] = np.array([0, 255, 255])    # Low vegetation
        cmap[3] = np.array([0, 255, 0])      # Tree
        cmap[4] = np.array([255, 255, 0])    # Car
        cmap[5] = np.array([255, 0, 0])      # Clutter/background
        cmap[255] = np.array([0, 0, 0])      # ignore
    return cmap
...
class meanIOU:
    ...
    def classPixelAccuracy(self):
        classAcc = np.diag(self.hist) / self.hist.sum(axis=1)
        return classAcc 

    def userAccuracy(self):
        classAcc = np.diag(self.hist) / self.hist.sum(axis=0)
        return classAcc 

    def meanUserAccuracy(self):
        classAcc = self.userAccuracy()
        meanAcc = np.nanmean(classAcc) 
        return meanAcc # 返回总体的UA平均值
    ...
    def KappaConsistency(self):
        ...
        row_sums = self.hist.sum(axis=1)
        col_sums = self.hist.sum(axis=0)
        all_sums_sq = self.hist.sum()*self.hist.sum()
...
def pre_slide(model, image, num_classes=2, tile_size=(420,420), tta=False):
    ...
    with torch.no_grad():
        for img, mask, id in loader:
            ...
            if mode == 'sliding_window':
                ...
                # print(f'img shape is{img.shape[-2: ]}')
                # print(f'mode is {mode}')
                pred = model(img).argmax(1)

            metric.add_batch(pred.cpu().numpy(), mask.numpy())

    iou_class, mIOU = metric.mIoUevaluate()
    OA = metric.pixelAccuracy()
    PA = metric.classPixelAccuracy()
    MPA = metric.meanPixelAccuracy()
    UA = metric.userAccuracy()
    MUA = metric.meanUserAccuracy()
    Kappa = metric.KappaConsistency()
    mIOU = mIOU * 100.0

    return mIOU, iou_class, OA, PA, MPA, UA, MUA, Kappa
