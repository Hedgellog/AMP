...
os.chdir("/home/lab532/llog-AMP-TGRS/ours_method")
...
def parse_args():
    ...
    parser.add_argument('--resume_model', type=str,
                        default='/media/llog/AGMM-SASSdata/model_files/vaihingen/SUBMIT/ablation_wi_MBPLandHFFI_point/DINOV2b_65.65.pth')
    parser.add_argument('--config', type=str, default='/home/lab532/llog-AMP-TGRS/AMP-TGRS/configs/vaihingen.yaml')
...
def get_dataset(cfg):
    if cfg['dataset'] == 'treecanopy':
        ...
        valset = VaihingenDataset(cfg['dataset'], cfg['data_root'], 'val', None)
    else:
        valset = None
...
def main(args):
    ...
    with torch.no_grad():
        for img, id in tbar:
            img = img.cuda()
            if cfg['dataset'] == 'treecanopy' or cfg['dataset'] == 'vaihingen':
