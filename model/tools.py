...
def build_cur_cls_label(mask, nclass):
    ...
    # 将one-hot编码的mask展平，并在最后一个维度上取最大值
...
def clean_mask(mask, cls_label, softmax=True):
    if softmax:
        ...
        # mask的输入参数是pred （b,k,h,w)即在k维度上做softmax
    n, c = cls_label.size()
    # cls_label(b, k),k是类别数，即n是batch，c是类别数
    """Remove any masks of labels that are not present"""
    return mask * cls_label.view(n, c, 1, 1)
    # 结果是（n,c,h,w）保留有效的类别标签，即忽视未标注的标签
...
def GMM(feat, vecs, pred, true_mask, cls_label):  # feat 输入特征图，vecs 向量，特征中心（参考protypes）， pred:预测结果， ture_mask真值掩码，cls_label 类别标签
    b, k, oh, ow = pred.size()                    # 得到输出的尺寸， batch，channel，h，w

    preserve = (true_mask < 255).long().view(b, 1, oh, ow)
    preserve = F.interpolate(preserve.float(), size=feat.size()[-2:], mode='bilinear')
    pred = F.interpolate(pred, size=feat.size()[-2:], mode='bilinear')          # pred和preserve 被插值到feat相同的尺寸
    _, _, h, w = pred.size()

    vecs = vecs.view(b, k, -1, 1, 1)            # 被视作一个中心向量
    feat = feat.view(b, 1, -1, h, w)            # 被视作一个特征图
    ...
    res = F.interpolate(res, size=(oh, ow), mode='bilinear')    ## 通过使用高斯分布的形式，代码尝试衡量特征与中心向量之间的匹配程度
